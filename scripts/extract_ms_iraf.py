...
for sp in opsps:
    ...
    phc.savefig(fig, figname=sp.replace('.fits', '.orders'))
    ...
    final = final/pflx
    spt.writeFits(final, swl, 
        savename=sp.replace('.fits', '.ext.fits'), quiet=False, externhd=sp )

    fig, ax = plt.subplots()
    ax.plot(swl, final)
    ax.set_ylim([0, 3])
    phc.savefig(fig, figname=sp.replace('.fits', ''))
