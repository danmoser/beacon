...
# Delete fits images(maintain only the original files and cp* files)
	if(swap)
		...
		if(swap) 
			imrename(oldnames="@"//fname,newnames="c//@"//fname,verbose=no)
		else
			imcopy(input="@"//fname,output="c//@"//fname,verbose=no)
	}
...
if(access("shifts")) {		
	...
	imalign(input="@"//fname,reference=imagem,
     	   coords="coordtmp",output="sh//@"//fname,shifts="shifts",
		   boxsize=boxsize,bigbox=bigbox,trimimages=no,shiftimages=yes)
} else {
	imalign(input="@"//fname,reference=imagem,
     	   coords="coordtmp",output="sh//@"//fname,shifts="",
		   boxsize=boxsize,bigbox=bigbox,trimimages=no,shiftimages=yes)
}
delete("coordtmp", ver-, >& "dev$null")
delete(fname, ver-, >& "dev$null")
...
imsum(input="shc*.fits",output="../"//temp0, title="",hparams="",pixtype="double",
                                  calctype="double",option="sum",verbose=no)
...
# BEDNARSKI, 15jul22: Tive de comentar esse filecalc, pois não funciona no meu IRAF 2.16. Ele não faz diferença.
...
#$
		...
		if(access("shifts")) {		
			print("# Using 'shifts' file present in the folder...")
			imalign(input="@"//fname,reference=imagem,
		        coords="../"//temp2//".ord",output="sh//@"//fname,shifts="shifts",
			   boxsize=boxsize,bigbox=bigbox,trimimages=no,shiftimages=yes)
		} else {
			imalign(input="@"//fname,reference=imagem,
		        coords="../"//temp2//".ord",output="sh//@"//fname,shifts="",
			   boxsize=boxsize,bigbox=bigbox,trimimages=no,shiftimages=yes)
		}
...
#$
                ...
                fname=mktemp("lista")
                files("sum*"//version//".fits", > fname)
		imalign(input="@"//fname,
			   reference="sum"//raiz//version//".fits",
                           coords="coordtmp",
			   output="sh//@"//fname,shifts="",
			   boxsize=boxsize,bigbox=bigbox,trimimages=no,shiftimages=yes)
...
# BEDNARSKI 06-06-2010: Deixa uma versao final das coordenadas no caso intera==no(o que ainda não havia sido feito)
		if(intera==no)
			rename("tempcoord.ord", "coord"//version//".ord")
	}
delete(temp1, ver-, >& "dev$null")
#$
delete("coordtmp.ord", ver-, >& "dev$null")
